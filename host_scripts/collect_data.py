...
def main():
    ...
    parser.add_argument('-d', "--debug", help="Enable debug messages", default=True, action="store_true")
    ...
    for machine_name in os.listdir(abs_root_data):
        ...
        if os.path.isdir(vm_path):
            ...
            if machine_name == "bardiel":
                ...
                gpu_number = 0
                gpu_type = "-"
            elif machine_name in {"aiteam", "1d8a70c2cede", "westworld", "3e2d1e1081a4"}:
                gpu_number = 8
                gpu_type = "GTX 1080Ti"
            ...
            for app in os.listdir(vm_path):
                ...
                if os.path.isdir(app_path) and os.path.exists(os.path.join(apps_path, app + ".py")) and (not args.app or args.app == app):
                    ...
                    if app in {"tf", "pytorch"}:
                        sort_command = "python3.6 " + sort_script + " -i " + app + ".csv -o " + app + ".csv -c 0,14"
                    else:
                        sort_command = sort_script + " -i " + app + ".csv -o " + app + ".csv -c 0"
                    subprocess.call(sort_command, shell=True, executable="/bin/sh")
