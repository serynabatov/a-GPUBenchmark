...
def compute_configuration_name(cl_parameters):
    ...
    if network_type == "resnet":
        if "n" in parameters:
            ...
            deep = "5"
        network_type = "resnet_" + str(deep) + "_deep"
...
def collect_data(repetition_path, gpu_type, gpu_number, debug):
    ...
    try:
        ...
        if os.path.exists(iteration_file_name) and not os.path.exists(iteration_file_name.replace(".csv", ".pdf")):
            create_graph_command = "python3.6 " + os.path.join(abs_root, "pytorch", "generate_iteration_graph.py") + " " + iteration_file_name + " -o " + iteration_file_name.replace(".csv", ".pdf") + " -s" + repetition_path
        ...
        if os.path.exists(profile_file_name_cpu) and os.path.exists(profile_file_name_gpu) and not os.path.exists(profile_file_name):
            ...
            create_graph_command = "python3.6 " + create_graph_command
            logging.debug("Executing %s", create_graph_command)
            return_value = subprocess.call(create_graph_command, shell=True, executable="/bin/bash")
            if return_value != 0:
        ...
        if network_type == "resnet":
            n_value = xml_configuration_file_name.split("_")[2]
        ...
        lr = xml_configuration.get("lr")
        if lr is None:
            lr = 1e-4

        optimizer = xml_configuration.get("optimizer")
...
def main():
    ...
    #Get patience
    patience = root.get("patience")
    if patience is None:
        logging.error("patience not set in xml file")
        sys.exit(1)

    patience_delta = root.get("patience_delta")
    if patience_delta is None:
    ...
    lr = root.get("lr")
    if lr is None:
       logging.error("learning rate not set in xml file")
       sys.exit(1)

    optimizer = root.get("optimizer")
    if optimizer is None:
       logging.error("optimizer not set in xml file")
       sys.exit(1)
    ...
    if gpus_instances is not None:
        ...
        # automatically gets the most available gpus
        export_gpus_command = "CUDA_VISIBLE_DEVICES=" + " ".join([str(gpu) for gpu in GPUtil.getAvailable(order='first', limit=1, maxLoad = 0.5, maxMemory = 0.5)])
    else:
        export_gpus_command = "CUDA_VISIBLE_DEVICES=" + " ".join([str(gpu) for gpu in GPUtil.getAvailable(order='first', limit=int(gpus_number), maxLoad = 0.5, maxMemory = 0.5)])
